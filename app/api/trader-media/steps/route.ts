...
import { nextSortOrder, serialiseStep } from "@/lib/trader-media/server";
...
export const POST = route(async (req) => {
  ...
  const phase = await prisma.traderMediaPhase.findUnique({
    where: { id: phaseId },
    include: { steps: { select: { sortOrder: true } } },
  });
  if (!phase) throw badRequest("That phase no longer exists");

  const customCount = await prisma.traderMediaStep.count({
    where: { phaseId, isCustom: true },
  });

  const step = await prisma.traderMediaStep.create({
    data: {
