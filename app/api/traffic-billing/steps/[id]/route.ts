...
import { serialiseStep } from "@/lib/traffic-billing/server";
 ...
 * step deliberately leaves every past month's tick untouched, since the state
...
export const PATCH = route(async (req, ctx) => {
  ...
  const step = await prisma.trafficBillingStep.update({
    where: { id },
    data: {
...
export const DELETE = route(async (_req, ctx) => {
  const id = await paramId(ctx);
  await prisma.trafficBillingStep.delete({ where: { id } });
  return { ok: true };
});
