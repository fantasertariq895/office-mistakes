...
import { progressFor, stateOf, type StateMap } from "@/lib/trader-media/progress";
import type { TmIssue, TmMistake, TmPhase } from "@/lib/types";
import {
  IconAlert,
  IconArrowLeft,
  IconArrowRight,
...
import { TmProgressBar, TmProgressLabel } from "./ProgressBar";
import { StepRow } from "./StepRow";

type Handlers = {
  setStepState: (stepId: number, next: TmStepState) => void;
  ...
  setPhaseState: (phaseId: number, next: TmStepState) => void;
...
export function PhaseCard({
   ...
   * label so "Update PIO Data" prints once, above its block, the way it
   * reads in the SOP.
   */
  const groups = useMemo(() => {
    const out: { label: string | null; steps: TmPhase["steps"] }[] = [];
  ...
  /**
   * Soft gating, not hard — same reasoning as Traffic Billing: warn once,
   * then let them through, rather than teaching people to tick boxes they
   * haven't done.
   */
  const handleNext = () => {
        ...
        <div className="tb-phase-heading">
          <span className="tb-phase-badge">
            ...
            {phase.isOwnerPending && (
              <span className="badge warning" title="Yuvika owns this step for now">
                Not yet yours
              </span>
            )}
          </span>
        ...
        {!readOnly && (
          <div className="tb-phase-bulk">
            ...
            <button
              ...
              title="This whole phase doesn't apply this week"
            >
              Phase N/A
            </button>
            <button
